...
//所有代码都包含在一个立即调用的匿名函数中,在文件加载完毕后立刻执行初始化各模块
//为什么要创建这样一个匿名函数
//防止与已有函数冲突,并确保不被外界函数变量干扰
//最后通过window.jQuery = window.$ = jQuery手动将jQuery绑定到window对象上,明确使得jQuery成为全局变量,同时保证其他部分私有
(function( window, undefined ) {
	//为什么要设置window参数,并传入window对象
	//原因一:这样可以将window对象变成局部对象,而无需每次访问window对象都要回退到作用域链顶部
	//原因二:可以便于代码压缩, function(a,b){这样a就相当于window对象了}
	//undefined原因相同
...
var jQuery = function( selector, context ) {
	...
	// 这里将可能已经存在在window的jQuery和$全局变量进行保存,防止覆盖
	...
	//匹配单一的标签, 即没有子标签,	可以是单个<div>/<img />/<div></div>
	rsingleTag = /^<(\w+)\s*\/?>(?:<\/\1>)?$/,
...
jQuery.fn = jQuery.prototype = {
	...
	init: function( selector, context, rootjQuery ) {
		...
		if ( typeof selector === "string" ) {
			...
			/**
			 * 验证match,看它是否是HTML标签
			 * 判断标准: 1/首先检测match[1],上面已经说明了为什么match[1]可以代表html标签
			 *          2/可以检测context的原因在于代码127行为html元素设置了context, 而id没有设置
			 */
